...
export async function POST(request) {
  try {
    ...
    if (process.env.RESEND_API_KEY) {
      // IMPORTANT: 'onboarding@resend.dev' can ONLY send to your own verified email.
      // To send to customers, add your own domain in Resend and set RESEND_FROM_EMAIL.
          ...
          html: `
            <div style="font-family: sans-serif; max-width: 600px; margin: auto;">
              ...
              <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
                <tr><td style="padding: 8px; border-bottom: 1px solid #e5e7eb;"><strong>Service:</strong></td><td style="padding: 8px; border-bottom: 1px solid #e5e7eb;">${serviceName}</td></tr>
                <tr><td style="padding: 8px; border-bottom: 1px solid #e5e7eb;"><strong>Date:</strong></td><td style="padding: 8px; border-bottom: 1px solid #e5e7eb;">${formattedDate}</td></tr>
                <tr><td style="padding: 8px; border-bottom: 1px solid #e5e7eb;"><strong>Time:</strong></td><td style="padding: 8px; border-bottom: 1px solid #e5e7eb;">${time} JST</td></tr>
                ...
                ${phone ? `<tr><td style="padding: 8px; border-bottom: 1px solid #e5e7eb;"><strong>Phone:</strong></td><td style="padding: 8px; border-bottom: 1px solid #e5e7eb;">${phone}</td></tr>` : ''}
              </table>
              ${notes ? `<h3>Notes:</h3><p style="background:#f8f7f4;padding:12px;border-left:3px solid #8B0000;white-space:pre-wrap;">${notes}</p>` : ''}
              <p style="color:#6b7280;font-size:12px;">Reply to this email to contact ${name} directly.</p>
            </div>
          `,
        }),
      });
      ...
      // Customer confirmation — only works with a verified custom domain
      if (process.env.RESEND_FROM_EMAIL && process.env.RESEND_FROM_EMAIL !== 'onboarding@resend.dev') {
        const customerRes = await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            from: `Yama Vista <${process.env.RESEND_FROM_EMAIL}>`,
            to: [email],
            reply_to: adminEmail,
        ...
        if (!customerRes.ok) {
          console.error('Resend customer email error:', JSON.stringify(customerResult));
        }
      }
    } else {
      console.warn('RESEND_API_KEY not configured — email not sent. Booking:', { service: serviceName, name, email, date, time });
    }
