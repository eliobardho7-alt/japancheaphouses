...
// With memberships off, nobody needs to be signed in to read anything.
const FREE_ACCESS = { user: null, isSubscribed: true, isAdmin: false };
...
export async function getViewerAccess() {
  // Everything is free right now, so skip the session and subscription lookups
  // entirely — this also keeps the premium pages statically cheap.
  if (!MEMBERSHIPS_ENABLED) return FREE_ACCESS;
