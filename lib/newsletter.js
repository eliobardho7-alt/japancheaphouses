 ...
 */
export async function storeSubscriber(email, source) {
  ...
  try {
    ...
    let { error } = await supabase
      ...
      .upsert(source ? { ...row, source } : row, { onConflict: 'email' });

    // Older copies of the table have no `source` column. Rather than losing
    // the subscriber over a reporting field, drop it and store the address.
    if (error && source && /source/i.test(error.message || '')) {
      console.warn('Newsletter: no `source` column; storing without it');
